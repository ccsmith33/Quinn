...
def test_drain_invalidates_deferred_rows_for_closed_positions() -> None:
    """Unreplayed rows whose position is gone are invalidated identically,
    even with zero active virtual exits (stale-prod-DB self-heal)."""
    deferred = [_deferred(7, virtual_exit_id=99, symbol="GONE")]
    ...
    assert report.invalidated_deferred == 1
    assert journal.skipped_deferred[0][0] == 7
